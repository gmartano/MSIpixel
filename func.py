...
def get_imz_parameter(xml_path):
    ...
    #print(f"Matrix {width}x{height} and pixel scan time is equal to {str(spacing/velocity)[:4]} ms")
...
'''
adducts = np.array([["H", 0, 0, 0, "+"], ["Na", 22.989769282, 0, 0, "+"], ["K", 38.9637064864, 0, 0, "+"],
                    ["Ca", 39.962590863, 0, 0, "+"], ["Fe", 55.93493633, 0, 0, "+"], ["Cu", 62.92959772, 0, 0, "+"],
                    ["H", 0, 0, 0, "-"],["Cl", 34.968852682, 0, 0, "-"],["Br", 78.9183376 , 0, 0, "-"],
                    ["H2PO4", 96.96907054116001 , 0, 0, "-"]], dtype=object)
...
if __name__ in "__main__":
    ...
    def multiprocess_mf(db, ppm, ms1, ms2):
        ...
        for i in i_range:
            ext2 = ms2[ms2[:,0] == i]
            ext1 = indexrange(ms1, i)
            args.append((ext2, ext1, ppm, db))
        for result in pool.imap_unordered(splitter, args):
            if len(result) > 0:
                for elem in result:
                    results.append(elem)
        pool.close()
    ...
    def multiprocess_annotation(mfs, ms2, lib):
        ncpu = psutil.cpu_count(logical=False) - 1
        args = []
        ...
        pool = mp.Pool(ncpu)
        for result in pool.imap_unordered(perform_annotation, args):
            if len(result) > 0:
                for elem in result:
                    results.append(elem)
        pool.close()
    ...
    def indexrange(spec, id_spec, parallel):
        if parallel == True:
    ...
    def correct_duplicates(best_score, ms2):
        ### add multiprocessing
        ...
        for i in index:
            ...
            if len(set(one_name[:,0])) > 1:
                one_name = resolving_duplicates(one_name, ms2)
            else:
                clean = np.concatenate((clean, one_name))
        
        return clean
    
    def eic_all_mp(xml, ppm, ms1, ms2):
        ncpu = 5
        args = []
        if ncpu < 2:
            args = (xml, ppm, ms1, ms2, None, True)
            return eic_all2(args)
        else:
            pace = np.arange(0, len(ms2), max(2, int(len(ms2)/ncpu)))
            pace[-1] = len(ms2)
            print(pace)
            for i in range(1, len(pace)):
                partial_ms2 = ms2[pace[i-1]:pace[i]]
                minmz, maxmz = min(partial_ms2[:,2]) - 1 , max(partial_ms2[:,2]) + 1
                partial_ms1 = mzrange(ms1, minmz, maxmz)
                args.append((xml, ppm, partial_ms1, partial_ms2, None, False))
            results_eic, results_eim = np.zeros((0,5), dtype=object), np.zeros((0,5), dtype=object)
            pool = mp.Pool(ncpu)
            for result_eic, result_eim in pool.imap_unordered(eic_all2, args):
    ...
    def run_infusion(mzml_path, lib_folder, ppm, xml):
        norm_tic = True
        lib = load_library_folder(lib_folder)
        db = db_from_lib(lib)
        
        ms1, ms2 = run_shared(mzml_path, norm_tic)
        index = [i for i,k in enumerate(ms2)]
        ms2 = np.c_[ms2, index]
        mfs = multiprocess_mf(db, ppm, ms1, ms2)
        ms2 = multiprocess_annotation(mfs, ms2, lib)
        
        clean = dynamic_resolver(ms2, ppm)
        best_s = best_score(clean)
        ms2 = correct_duplicates(best_s, ms2)
        ms2 = ms2[ms2[:,2].argsort()]
        all_eic, all_eim = eic_all_mp(xml, ppm, ms1, ms2)
        return all_eic, all_eim 
        


    '''
    mzml_path = "C:\\Users\\giuse\\OneDrive\\Desktop\\test\\HCCA_MS2.mzML"
    xml = "C:\\Users\\giuse\\OneDrive\\Desktop\\test\\HCCA_MS2.xml"
    lib_folder = "C:\\Users\\giuse\\OneDrive\\Desktop\\Library\\"
    ppm = 5
    norm_tic = True
    all_eic, all_eim  = run_infusion(mzml_path, lib_folder, ppm, xml)
    '''
    def transform_data_tic(mzml_file):
        print("Extracting ms and ms2 values")
        run = pymzml.run.Reader(mzml_file)
        #msi datastructure = [index, mslevel, precursor, time, mz, intensities, polarity]
        index = 0
        ...
        for n, spec in enumerate(run):
            if spec["negative scan"] is not None:
                ...
                polarity = "+"
            if spec.ms_level == 1:
                ms.append([index, spec.ms_level, None, spec.scan_time_in_minutes()*60, spec.mz, spec.TIC, polarity])
                index += 1
        return np.array(ms, dtype=object)

    mzml_path = "C:\\Users\\giuse\\OneDrive\\Desktop\\Manuscript MSIpixel\\test\\HCCA_MS2.mzML"
    xml_path = "C:\\Users\\giuse\\OneDrive\\Desktop\\Manuscript MSIpixel\\test\\HCCA_MS2.xml"
    ms1 = transform_data_tic(mzml_path)
    args = (get_imz_parameter(xml_path))
    pixel_time, transitionTime, width, height = args
    xyt = imz_matrix(width, height, pixel_time, transitionTime)
    imz = []
    for i in range(len(xyt)):
        ...
        if len(pixel) == 0:
            ...
            value_i = np.mean(pixel[:,5])
        else:
            value_i = 0
        imz.append([xyt[i][0], xyt[i][1], value_i])
    imz = np.array(imz, dtype=object)
    matrix = plot_data(imz)
    plt.imshow(matrix)
