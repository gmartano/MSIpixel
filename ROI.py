...
class Ui_Form(object):
    ...
    def export_pixels(self):
        if len(self.selected_boxplot) > 0:
            ...
            save_path = QtWidgets.QFileDialog.getSaveFileName(None, "Save CSV file as", "", ".txt(*.txt)")[0]
            with open(save_path, 'a') as f:
                df_string = col_names.to_string(header=False, index=False)
                col_names.to_string()
                f.write(df_string)
            f.close()
            save_path_parq = str(str.split(save_path, ".")[0] + ".parquet")
            col_names.columns = col_names.columns.astype(str)
            col_names.to_parquet(save_path_parq)
            #col_names.to_csv(save_path, index=False, header=False)
    ...
    def line_drawer(self):
        ...
        while [int(i) for i in xy[0]] != [int(i) for i in start[0]]:
            xy = plt.ginput(1)
            lines.append([int(i) for i in xy[0]])
            x = [int(p[0]) for p in lines[-2:]]
            y = [int(p[1]) for p in lines[-2:]]
            plt.plot(x,y, color=self.line_color, linewidth=self.linewidth)
            self.canvas.draw()
...
if __name__ == "__main__":
    import sys
    import numpy as np
    path = "C:\\Users\\giuse\\OneDrive\\Desktop\\test\\HCCA_MS2\\all_eim.npy"
    #path = "C:\\Users\\Utente\\OneDrive\\Desktop\\test\\HCCA_MS2\\all_eim.npy"
    all_eim = np.load(path, allow_pickle=True)
    r_arr = func.plot_data(all_eim[0,2])
    app = QtWidgets.QApplication(sys.argv)
    Form = QtWidgets.QWidget()
    ui = Ui_Form()
    ui.setupUi(Form, all_eim)
    Form.show()
    sys.exit(app.exec_())
'''